...
public:
    ...
    void ReadArray(std::vector<std::string>& array, uint32_t size) {
        ...
        for (uint32_t i = 0; i < size; i++) ReadString(array[i]);
    }
};
...
public:
    ...
    template <typename Key, typename Value>
    void ReadMapBigEndian(std::map<Key, Value>& map, uint32_t size = 0) {
        ReadMap(map, size);
    }
...
private:
    template <typename T>
    void SwapEndian(T* data, size_t size) {
...
public:
    ...
    bool ReadData(char* destination, size_t size) override {
        if (m_Position + size > m_Buffer.size()) {
            return false;  // Trying to read beyond buffer size
        }
