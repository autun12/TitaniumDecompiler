...
struct StackMapFrameType {
    ...
    virtual void Deserialize(BigEndianStreamReader* deserializer,
                             StackMapFrameType& instance) = 0;
};

struct StackMapFrame {
public:
    StackMapFrame() : m_Type(0), m_StackFrameType(nullptr) {}
    StackMapFrame(uint8_t type, StackMapFrameType* stackFrameType)
        : m_Type(type), m_StackFrameType(stackFrameType) {}

    ~StackMapFrame() = default;

    static void Deserialize(BigEndianStreamReader* deserializer,
                            StackMapFrame& instance);

private:
    std::shared_ptr<StackMapFrameType> CreateStackFrameFromTag(
        StackMapFrame& instance);
...
struct StackMapSame : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapSame& stackMapSameInstance =
            static_cast<StackMapSame&>(instance);
    }
};
...
struct StackMapSameLocals1StackItemFrame : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapSameLocals1StackItemFrame&
            stackMapSameLocals1StackItemFrameInstance =
                static_cast<StackMapSameLocals1StackItemFrame&>(instance);
        Deserialize(deserializer, stackMapSameLocals1StackItemFrameInstance);
    };

    static void Deserialize(BigEndianStreamReader* deserializer,
                            StackMapSameLocals1StackItemFrame& instance) {
...
struct StackMapSameLocals1StackItemFrameExtended : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapSameLocals1StackItemFrameExtended&
            stackMapSameLocals1StackItemFrameExtendedInstance =
                static_cast<StackMapSameLocals1StackItemFrameExtended&>(
                    instance);
    ...
    static void Deserialize(
        BigEndianStreamReader* deserializer,
        StackMapSameLocals1StackItemFrameExtended& instance) {
...
struct StackMapChopFrame : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapChopFrame& stackMapChopFrameInstance =
            static_cast<StackMapChopFrame&>(instance);
        Deserialize(deserializer, stackMapChopFrameInstance);
    };

    static void Deserialize(BigEndianStreamReader* deserializer,
                            StackMapChopFrame& instance) {
        deserializer->ReadRawBigEndian<uint16_t>(instance.offsetDelta);
    }
    uint16_t offsetDelta;
};
...
struct StackMapSameFrameExtended : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapSameFrameExtended& stackMapSameFrameExtendedInstance =
            static_cast<StackMapSameFrameExtended&>(instance);
        Deserialize(deserializer, stackMapSameFrameExtendedInstance);
    };

    static void Deserialize(BigEndianStreamReader* deserializer,
                            StackMapSameFrameExtended& instance) {
        deserializer->ReadRawBigEndian<uint16_t>(instance.offsetDelta);
    }
    uint16_t offsetDelta;
};
...
struct StackMapAppendFrame : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapAppendFrame& stackMapAppendFrame =
            static_cast<StackMapAppendFrame&>(instance);
        deserializer->ReadRawBigEndian<uint16_t>(
            stackMapAppendFrame.offsetDelta);
        if (m_FrameType - 251 > 0) {
            deserializer->ReadArrayBigEndian(stackMapAppendFrame.locals,
                                             m_FrameType - 251);
        }
    };
...
struct StackMapFullFrame : StackMapFrameType {
    ...
    void Deserialize(BigEndianStreamReader* deserializer,
                     StackMapFrameType& instance) override {
        StackMapFullFrame& stackMapFullFrameInstance =
            static_cast<StackMapFullFrame&>(instance);
        Deserialize(deserializer, stackMapFullFrameInstance);
    };

    static void Deserialize(BigEndianStreamReader* deserializer,
                            StackMapFullFrame& instance) {
        deserializer->ReadRawBigEndian<uint16_t>(instance.offsetDelta);
        deserializer->ReadRawBigEndian<uint16_t>(instance.numOfLocals);

        if (instance.numOfLocals > 0) {
            deserializer->ReadArrayBigEndian(instance.locals,
                                             instance.numOfLocals);
        }

        deserializer->ReadRawBigEndian<uint16_t>(instance.numOfStackItems);

        if (instance.numOfStackItems > 0) {
            deserializer->ReadArrayBigEndian(instance.stack,
                                             instance.numOfStackItems);
        }
    }
    uint16_t offsetDelta;
