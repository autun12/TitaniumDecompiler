...
const Insn* CFG::FindInstructionByAddr(const InstrMap& instructions,
                                       uint64_t targetAddr) {
    ...
    for (auto it = instructions.begin(); it != instructions.end(); ++it, ++i) {
        ...
        switch (insn.group) {
            case GROUP_JUMP: {
                ...
                const Insn* targetInstr =
                    FindInstructionByAddr(instructions, jumpTargetAddr);
                ...
                if (targetInstr) {
                    ...
                    TD_DECOMP_WARN(
                        "Jump target at addr {0} not found in instruction map.",
                        jumpTargetAddr);
                }

                // Set boundary at the next instruction (end of current basic
                // block)
...
std::map<uint32_t, std::shared_ptr<BasicBlock>> CFG::CreateBasicBlocks(
    ...
    std::map<uint32_t, SSAVariable>
        localVarVersions;                   // Track local variable versions
    ...
    for (auto& [offset, insn] : instructions) {
        // If the offset is a boundary and the current block has instructions,
        // finalize the block
        if (boundaries.find(offset) != boundaries.end() &&
            !currentBlock->GetInstructions().empty()) {
        ...
        auto [ssaInstrs, updatedStack, updatedLocalVars] =
            TranslateBytecodeInstruction(insn, currentStack, localVarVersions);
        std::shared_ptr<SSABasicBlock> ssaBlock;
        if (blockIdToSSABlock.find(currentBlock->m_ID) ==
            blockIdToSSABlock.end()) {
...
// std::vector<std::shared_ptr<BasicBlock>> CFG::BuildBlocks(const InstrMap&
// instructions) {
//     std::map<uint32_t, std::shared_ptr<BasicBlock>> mapInstrBlocks;
//     std::vector<std::shared_ptr<BasicBlock>> basicBlocks;

//     // Step 1: Identify target addresses of all jumps and add them as
//     boundaries std::set<size_t> boundaries = FindBoundaries(instructions);

//     // Step 2: Create blocks using boundaries
//     std::shared_ptr<BasicBlock> currentBlock =
//     std::make_shared<BasicBlock>(); int blockID = 0;  // Block ID counter

//     // Iterate over the instructions (the multimap is ordered by offset)
//     for (auto& [offset, insn] : instructions) {
//         // If the offset is a boundary and the current block has
//         instructions, finalize the block if (boundaries.find(offset) !=
//         boundaries.end() && !currentBlock->GetInstructions().empty()) {
//             currentBlock->m_ID = blockID++;  // Assign ID and increment
//             basicBlocks.push_back(currentBlock);
//             m_Blocks[currentBlock->m_ID] = currentBlock;
//             currentBlock = std::make_shared<BasicBlock>();
...
void CFG::PrintBasicBlocks(
    ...
    for (const auto& block : basicBlocks) {
        ...
        for (const auto& insn : block->GetInstructions()) {
            TD_DECOMP_INFO(" - Opcode: {0}, Name: {1}", (int)insn.opcode,
                           insn.opcodeName);
        }
...
void CFG::ConnectBasicBlocks(
    const std::map<uint32_t, std::shared_ptr<BasicBlock>>& mapInstrBlocks) {
...
void CFG::BuildGraph() {
    for (auto& [index, block] : m_Blocks) {
        ...
        // size_t targetIndex = lastInsn.Op1.addr;  // Target of the jump (now
        // stored in Op1.addr) printf("target index %d\n", targetIndex);
        ...
        // }
        // }
        // For branch instructions, connect both the branch target and the next
        // sequential block else if (IsBranchInstruction(lastInsn)) { size_t
        // targetIndex = lastInsn.Op1.addr; if (m_Blocks.find(targetIndex) !=
        // m_Blocks.end()) { auto targetBlock = m_Blocks[targetIndex];
        // block->AddSuccessor(targetBlock);
        // }
        // if (m_Blocks.find(index + 1) != m_Blocks.end()) {  // Next sequential
        // block auto nextBlock = m_Blocks[index + 1];
        // block->AddSuccessor(nextBlock);
        // }
        // }
        // For all other instructions, connect to the next sequential block if
        // not a return
...
bool CFG::IsBranchInstruction(const Insn& insn) {
    return insn.group == GROUP_JUMP;
}

bool CFG::IsReturnInstruction(const Insn& insn) {
    return insn.group == GROUP_RETURN;
}

bool CFG::IsSwitchInstruction(const Insn& insn) {
    return (insn.opcode == OPCODE_TABLESWITCH ||
            insn.opcode == OPCODE_LOOKUPSWITCH);
}

bool CFG::IsConditionalBranch(const Insn& insn) {
    return (
        insn.opcode == OPCODE_IFEQ || insn.opcode == OPCODE_IFNE ||
        insn.opcode == OPCODE_IFLT || insn.opcode == OPCODE_IFGE ||
        insn.opcode == OPCODE_IFGT || insn.opcode == OPCODE_IFLE ||
        insn.opcode == OPCODE_IF_ICMPEQ || insn.opcode == OPCODE_IF_ICMPNE ||
        insn.opcode == OPCODE_IF_ICMPLT || insn.opcode == OPCODE_IF_ICMPGE ||
        insn.opcode == OPCODE_IF_ICMPGT || insn.opcode == OPCODE_IF_ICMPLE ||
        insn.opcode == OPCODE_IF_ACMPEQ || insn.opcode == OPCODE_IF_ACMPNE ||
        insn.opcode == OPCODE_IFNULL || insn.opcode == OPCODE_IFNONNULL);
}

bool CFG::IsUnconditionalJump(const Insn& insn) {
    return (insn.opcode == OPCODE_GOTO || insn.opcode == OPCODE_GOTO_W);
}

bool CFG::IsInvokeCall(const Insn& insn) {
    return (insn.opcode == OPCODE_INVOKEVIRTUAL ||
            insn.opcode == OPCODE_INVOKEINTERFACE ||
            insn.opcode == OPCODE_INVOKESPECIAL);
}
...
std::tuple<std::vector<SSAInstruction>, std::vector<SSAVariable>,
           std::map<uint32_t, SSAVariable>>
CFG::TranslateBytecodeInstruction(
    ...
    std::vector<SSAVariable> updatedStack =
        stack;  // Start with the current stack
    std::map<uint32_t, SSAVariable> updatedLocalVars =
        localVarVersions;  // Start with current local variable versions

    switch (insn.opcode) {
        case OPCODE_BIPUSH: {
            int32_t byteValue =
                static_cast<int32_t>(insn.Op1.value);  // Get the byte value
            SSAVariable result(
                Type::BasicType(Type::Int, "int"),  // BIPUSH pushes an int
            ...
            ssaInstructions.push_back(
                {SSAInstruction::LOAD_CONST, {}, result});  // Use LOAD_CONST
        ...
        case OPCODE_ALOAD: {
            ...
            if (updatedLocalVars.find(index) != updatedLocalVars.end()) {
                ...
                localVariable =
                    SSAVariable(Type::ClassNameType("java/lang/Object"),
                                SSAVariable::Origin::LOCAL,
                                "local" + std::to_string(index), index);
            }
            SSAVariable result(localVariable.type,
                               SSAVariable::Origin::OPERATION, "aload_result");
            ssaInstructions.push_back(
                {SSAInstruction::LOAD_LOCAL, {localVariable}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ALOAD_0: {
        ...
        case OPCODE_IADD: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION,
                               "iadd_result");  // Unique name
            SSAInstruction addInstr(SSAInstruction::ADD, {operand1, operand2},
                                    result);
        ...
        case OPCODE_ICONST_M1: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_m1", -1);
        ...
        case OPCODE_ICONST_0: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_0", 0);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ICONST_1: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_1", 1);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ICONST_2: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_2", 2);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ICONST_3: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_3", 3);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ICONST_4: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_4", 4);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ICONST_5: {
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::CONSTANT, "iconst_5", 5);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_FCONST_0: {
            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::CONSTANT, "fconst_0", 0.0f);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_FCONST_1: {
            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::CONSTANT, "fconst_1", 1.0f);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_FCONST_2: {
            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::CONSTANT, "fconst_2", 2.0f);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_DCONST_0: {
            SSAVariable result(Type::BasicType(Type::Double, "double"),
                               SSAVariable::Origin::CONSTANT, "dconst_0", 0.0);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_DCONST_1: {
            SSAVariable result(Type::BasicType(Type::Double, "double"),
                               SSAVariable::Origin::CONSTANT, "dconst_1", 1.0);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_LCONST_0: {
            SSAVariable result(Type::BasicType(Type::Long, "long"),
                               SSAVariable::Origin::CONSTANT, "lconst_0", 0L);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_LCONST_1: {
            SSAVariable result(Type::BasicType(Type::Long, "long"),
                               SSAVariable::Origin::CONSTANT, "lconst_1", 1L);
            ssaInstructions.push_back({SSAInstruction::LOAD_CONST, {}, result});
            updatedStack.push_back(result);
            break;
        }
        case OPCODE_ASTORE_0: {
            ...
            updatedStack.pop_back();
            SSAInstruction storeInstr(SSAInstruction::STORE_LOCAL,
                                      {valueToStore});
        ...
        case OPCODE_INVOKEVIRTUAL: {
            MethodRefStack methStack =
                GetMethodRefFromIndex(m_ClassFile, insn.Op1.value);
            ...
            SSAVariable result(
                Type::ClassNameType("java/lang/Object"),
                SSAVariable::Origin::OPERATION,
                "invokevirtual_" + methStack.m_Class + "." +
                    methStack.m_Name);  //  Determine the actual return type.
        ...
        case OPCODE_INVOKESPECIAL: {
            ...
            SSAVariable result(
                Type::ClassNameType("java/lang/Object"),
                SSAVariable::Origin::OPERATION,
                "invokespecial_" + methStack.m_Class + "." +
                    methStack.m_Name);  //  Determine the actual return type.
            ssaInstructions.push_back({SSAInstruction::CALL, args, result});
            updatedStack.push_back(result);
            break;
        ...
        case OPCODE_INVOKESTATIC: {
            ...
            SSAVariable result(
                Type::ClassNameType("java/lang/Object"),
                SSAVariable::Origin::OPERATION,
                "invokestatic_" + methStack.m_Class + "." +
                    methStack.m_Name);  //  Determine the actual return type.
            ssaInstructions.push_back({SSAInstruction::CALL, args, result});
            updatedStack.push_back(result);
            break;
        ...
        case OPCODE_ILOAD: {
            ...
            if (updatedLocalVars.find(index) != updatedLocalVars.end()) {
                ...
                localVariable =
                    SSAVariable(Type::BasicType(Type::Int, "int"),
                                SSAVariable::Origin::LOCAL,
                                "local" + std::to_string(index), index);
            }

            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "iload_result");
            SSAInstruction loadInstr(SSAInstruction::LOAD_LOCAL,
        ...
        case OPCODE_I2S: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "short"),
                               SSAVariable::Origin::OPERATION, "i2s_result");
            SSAInstruction castInstr(SSAInstruction::CAST, {valueToConvert},
                                     result);
        ...
        case OPCODE_FLOAD_0: {
            ...
            if (updatedLocalVars.find(index) != updatedLocalVars.end()) {
                ...
                localVariable =
                    SSAVariable(Type::BasicType(Type::Float, "float"),
                                SSAVariable::Origin::LOCAL,
                                "local" + std::to_string(index), index);
            }

            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::OPERATION, "fload_result");
            SSAInstruction loadInstr(SSAInstruction::LOAD_LOCAL,
                                     {localVariable}, result);
            ssaInstructions.push_back(loadInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_LLOAD_0: {
            ...
            if (updatedLocalVars.find(index) != updatedLocalVars.end()) {
                ...
                localVariable =
                    SSAVariable(Type::BasicType(Type::Long, "long"),
                                SSAVariable::Origin::LOCAL,
                                "local" + std::to_string(index), index);
            }

            SSAVariable result(Type::BasicType(Type::Long, "long"),
                               SSAVariable::Origin::OPERATION, "lload_result");
            SSAInstruction loadInstr(SSAInstruction::LOAD_LOCAL,
                                     {localVariable}, result);
            ssaInstructions.push_back(loadInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_ISTORE: {
            ...
            SSAInstruction storeInstr(SSAInstruction::STORE_LOCAL,
                                      {valueToStore});
            ssaInstructions.push_back(storeInstr);
            updatedLocalVars[index] =
                valueToStore;  // Update local variable version
            break;
        }
        case OPCODE_FADD: {
            SSAVariable operand2 = updatedStack.back();
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::OPERATION, "fadd_result");
            SSAInstruction addInstr(SSAInstruction::ADD, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(addInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_LADD: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Long, "long"),
                               SSAVariable::Origin::OPERATION, "ladd_result");
            SSAInstruction addInstr(SSAInstruction::ADD, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(addInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_DADD: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Double, "double"),
                               SSAVariable::Origin::OPERATION, "dadd_result");
            SSAInstruction addInstr(SSAInstruction::ADD, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(addInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_ISUB: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "isub_result");
            SSAInstruction subInstr(SSAInstruction::SUB, {operand1, operand2},
                                    result);
        ...
        case OPCODE_IMUL: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "imul_result");
            SSAInstruction mulInstr(SSAInstruction::MUL, {operand1, operand2},
                                    result);
        ...
        case OPCODE_IDIV: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "idiv_result");
            SSAInstruction divInstr(SSAInstruction::DIV, {operand1, operand2},
                                    result);
        ...
        case OPCODE_IREM: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "irem_result");
            SSAInstruction remInstr(SSAInstruction::REM, {operand1, operand2},
                                    result);
        ...
        case OPCODE_FREM: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Float, "float"),
                               SSAVariable::Origin::OPERATION, "frem_result");
            SSAInstruction remInstr(SSAInstruction::REM, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(remInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_DREM: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Double, "double"),
                               SSAVariable::Origin::OPERATION, "drem_result");
            SSAInstruction remInstr(SSAInstruction::REM, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(remInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_LREM: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Long, "long"),
                               SSAVariable::Origin::OPERATION, "lrem_result");
            SSAInstruction remInstr(SSAInstruction::REM, {operand1, operand2},
                                    result);
            ssaInstructions.push_back(remInstr);
            updatedStack.push_back(result);
        ...
        case OPCODE_IFNE: {
            ...
            updatedStack.pop_back();
            SSAVariable result(Type::BasicType(Type::Boolean, "boolean"),
                               SSAVariable::Origin::OPERATION, "ifne_result");
            SSAInstruction ifneInstr(SSAInstruction::ICMP_NE_ZERO,
                                     {conditionVar}, result);
        ...
        case OPCODE_IINC: {
            ...
            SSAVariable incrementVar(
                Type::BasicType(Type::Int, "int"),
                SSAVariable::Origin::CONSTANT,
                "increment_const_" + std::to_string(increment), -1);
            SSAVariable result(Type::BasicType(Type::Int, "int"),
                               SSAVariable::Origin::OPERATION, "iinc_result");

            SSAInstruction addInstr(SSAInstruction::ADD,
                                    {localVariable, incrementVar}, result);
            ssaInstructions.push_back(addInstr);
        ...
        case OPCODE_NEWARRAY: {
            ...
            updatedStack.pop_back();
            //  In a real scenario, the type would be determined by
            //  insn.Op1.value
            Type arrayType = getType(insn.Op1.value);
            Type arr = Type::ReferenceType(std::make_unique<Type>(arrayType));
            SSAVariable result(arr, SSAVariable::Origin::OPERATION,
                               "newarray_result");
            SSAInstruction newArrayInstr(SSAInstruction::NEW_ARRAY, {size},
                                         result);
        ...
        case OPCODE_ANEWARRAY: {
            ...
            updatedStack.pop_back();
            // In a real scenario, the type would be determined by
            // insn.Op1.value For simplicity, we'll assume the type is
            // java.lang.Object.  In a full implementation, you'd use the type
            // information from the constant pool.
            auto ty =
                FieldDescriptor::newDescriptor(
                    GetConstantClassFromClass(m_ClassFile, insn.Op1.value))
                    .m_Ty;
            Type arrayType = Type::ReferenceType(std::make_unique<Type>(
                Type::ClassNameType("java/lang/Object")));
            SSAVariable result(ty, SSAVariable::Origin::OPERATION,
                               "anewarray_result");
            SSAInstruction newArrayInstr(SSAInstruction::NEW_ARRAY, {size},
                                         result);
            ssaInstructions.push_back(newArrayInstr);
        ...
        case OPCODE_AASTORE: {
            ...
            SSAInstruction arrayStoreInstr(SSAInstruction::ARRAY_STORE,
        ...
        case OPCODE_LDC: {
            ...
            if (cpi.Tag == Tags::String) {
                ...
                SSAVariable result(Type::ClassNameType("java/lang/String"),
                                   SSAVariable::Origin::CONSTANT,
                                   "ldc_string_" + constStr, -1);
                ssaInstructions.push_back(
                    {SSAInstruction::LOAD_CONST, {}, result});
                updatedStack.push_back(result);
            } else if (cpi.Tag == Tags::Int) {
                int32_t value = static_cast<int32_t>(insn.Op1.value);
                SSAVariable result(Type::BasicType(Type::Int, "int"),
                                   SSAVariable::Origin::CONSTANT,
                                   "ldc_int_" + std::to_string(value), -1);
                ssaInstructions.push_back(
                    {SSAInstruction::LOAD_CONST, {}, result});
                updatedStack.push_back(result);
                ...
                SSAVariable result(Type::BasicType(Type::Float, "float"),
                                   SSAVariable::Origin::CONSTANT,
                                   "ldc_float_" + std::to_string(value), -1);
                ssaInstructions.push_back(
                    {SSAInstruction::LOAD_CONST, {}, result});
                updatedStack.push_back(result);
            }
        ...
        case OPCODE_GETSTATIC: {
            auto fieldRefStack =
                GetFieldRefFromIndex(m_ClassFile, insn.Op1.value);
            SSAVariable result = SSAVariable(
                fieldRefStack.m_type, SSAVariable::Origin::OPERATION,
                "getstatic_" + fieldRefStack.m_Class + "." +
        ...
        case OPCODE_GOTO: {
            ...
            SSAInstruction gotoInstr(SSAInstruction::GOTO, {}, std::nullopt,
                                     branchOffset);  // No result
    ...
    //     case OPCODE_FSTORE: {
    //         ASTPtr store =
    //         std::make_shared<AST>(std::move(Store(instruction.Op1.value).value()));
    //         return store;
    //     }
    ...
    //             m_Stack.emplace_back(StackEntry::Array{arr.type, arr.length,
    //             arr.elements});
    //         } else {
    //             //if array is not explicitly an array, assume it is an object
    //             array. auto entry = ConvertStackEntryToAST(array);
    //             AST::ArrayIndex arrIndex{entry,
    //             ConvertStackEntryToAST(index), array.ty()}; AST::ReAssignment
    //             reAssign{ std::make_shared<AST>(arrIndex),
    //             ConvertStackEntryToAST(val)}; ASTPtr reAssignment =
    //             std::make_shared<AST>(std::move(reAssign)); return
    //             reAssignment;
    //         }
    //         break;
    //     }
    ...
    //     case OPCODE_LNEG: {
    //         StackEntry val = PopStack();
    //         m_Stack.emplace_back(StackEntry::UnaryOperation{UnaryOp::Neg,
    //         std::make_unique<StackEntry>(std::move(val))}); break;
    //     }
    //     //Process opcode for type checking
    //     case OPCODE_INSTANCEOF: {
    //         StackEntry obj1 = PopStack(); //pop object reference
    //         std::string obj2 = GetConstantClassFromClass(m_ClassFile,
    //         instruction.Op1.value); //Retrieve class name from constant pool
    //         StackEntry::Class obj2Class{obj2};
    //         StackEntry::BinaryOperation
    //         binOp{std::make_shared<StackEntry>(obj1), BinaryOp::InstanceOf,
    ...
    //         auto methStack = GetDynamicInvokeRefFromIndex(m_ClassFile,
    //         instruction.Op1.value);

    //         if(methStack.m_Class == "StringConcatFactory" && methStack.m_Name
    ...
    //         methStack.desc.returnType}; m_Stack.emplace_back(func); break;
    //     }
    //     //Process opcodes for interface method calls
    //     case OPCODE_INVOKEINTERFACE: {
    //         auto methStack = GetMethodRefFromIndex(m_ClassFile,
    //         instruction.Op1.value);
    ...
    //     case OPCODE_INVOKESPECIAL: {
    //         MethodRefStack methStack = GetMethodRefFromIndex(m_ClassFile,
    //         instruction.Op1.value); std::vector<StackEntry> argsStack;
    ...
    //         if(methStack.m_Name == "<init>" || methStack.m_Name ==
    //         "<clinit>") {
    //             if(objStackEntry.isClass()) {
    //                 methodName = objStackEntry.getClassValue().m_ClassName;
    //                 returnType = Type::ClassNameType(methStack.m_Class); //
    //                 Constructor returns the object type return
    //                 std::make_shared<AST>(AST::Object{methodName}); //
    ...
    //                 returnType = Type::ClassNameType(methStack.m_Class); //
    //                 Constructor returns the object type
    ...
    //             if(objStackEntry.isClass()) {
    //                 methodName = objStackEntry.getClassValue().m_ClassName +
    //                 "." + methStack.m_Name; returnType =
    //                 Type::ClassNameType(methStack.m_Class); // Assuming
    //                 method returns the class type for now - adjust based on
    //                 methStack.desc.return_type
    //             }
    //         }

    //         if (!methodName.empty()) {
    //             return std::make_shared<AST>(AST::FunctionCall{methodName,
    //             argsAST, returnType});
    //         } else {
    //             // Handle cases where method name couldn't be determined
    //             std::cerr << "Warning: Could not determine method name for
    //             INVOKESPECIAL" << std::endl; return nullptr; // Or some other
    ...
    //     case OPCODE_INVOKESTATIC: {
    //         MethodRefStack methStack = GetMethodRefFromIndex(m_ClassFile,
    //         instruction.Op1.value); std::vector<StackEntry> args; for(auto&
    //         descArgs : methStack.desc.args) {
    //             args.emplace_back(PopStack());
    //         }

    //         std::string funcName = methStack.m_Class + "." +
    //         methStack.m_Name; StackEntry::Function func{funcName, args,
    ...
    //             methStack.desc.returnType}; AST::MethodCall
    //             call{std::make_shared<AST>(object),
    //             std::make_shared<AST>(funcCall)}; ASTPtr methodCall =
    //             std::make_shared<AST>(std::move(call)); return methodCall;
    //         }
    ...
    //         methStack.desc.returnType}; if(methStack.desc.returnType.kind ==
    //         Type::Void) {
    ...
    //             AST::FunctionCall funcCall{methStack.m_Name, astArgs,
    //             methStack.desc.returnType}; AST::MethodCall
    //             call{std::make_shared<AST>(obj),
    //             std::make_shared<AST>(funcCall)}; ASTPtr methodCall =
    //             std::make_shared<AST>(std::move(call));
    ...
    //     }
    //     case OPCODE_GETFIELD: {
    //         auto fieldRefStack = GetFieldRefFromIndex(m_ClassFile,
    //         instruction.Op1.value); auto obj = PopStack();
    //         if(obj.isIdentifier()) {
    //             StackEntry::Field field{std::make_shared<StackEntry>(obj),
    //             fieldRefStack.m_Name, fieldRefStack.m_type};
    //             m_Stack.emplace_back(field);
    ...
    //         auto val = PopStack();
    //         FieldRefStack fieldRefStack = GetFieldRefFromIndex(m_ClassFile,
    //         instruction.Op1.value); AST::Object obj{fieldRefStack.m_Class};
    //         AST::FieldAssignment fieldAssign{std::make_shared<AST>(obj),
    //         fieldRefStack.m_Name, fieldRefStack.m_type,
    //         ConvertStackEntryToAST(val)}; ASTPtr fieldAssignment =
    //         std::make_shared<AST>(std::move(fieldAssign)); return
    //         fieldAssignment;
    //     }
    ...
    //         AST::FieldAssignment fieldAssign{ConvertStackEntryToAST(obj),
    //         fieldRefStack.m_Name, fieldRefStack.m_type,
    //         ConvertStackEntryToAST(val)}; ASTPtr fieldAssignment =
    //         std::make_shared<AST>(std::move(fieldAssign)); return
    //         fieldAssignment;
    ...
    //         StackEntry val = PopStack();
    //         StackEntry::UnaryOperation unaryOp{UnaryOp::ArrayLength,
    //         std::make_shared<StackEntry>(val)};
    ...
    //     case OPCODE_POP2: {
    //         auto val1 = PopStack();
    //         if(val1.ty().kind == Type::Long || val1.ty().kind ==
    //         Type::Double) { } else {
    ...
    //         if(then.size() == 1) {
    //             if (then[0]->isIfStatement()) { // Check if AST is an If
    //             condition
    ...
    //                 this_cond}; cond = std::make_shared<AST>(binOp);
    //                 // cond =
    //                 std::make_shared<AST>(AST::BinaryOperation(cond,
    ...
    //                 auto this_then = ifAst.then;
    //                 then = this_then;
    //                 AST::BinaryOperation binOp{cond,
    //                 BinaryOp::GreaterEqualThan, this_cond}; cond =
    ...
    //             ConvertStackEntryToAST(raw_cond)}; cond =
    //             std::make_shared<AST>(unOp);
    //         } else {
    ...
    //                then = this_if.then;
    //                AST::BinaryOperation
    //                binOp{ConvertStackEntryToAST(raw_cond),
    //                BinaryOp::LogicalOr, std::move(this_if.cond)}; cond =
    //                std::make_unique<AST>(binOp);
    //             } else {
    //                 AST::UnaryOperation unOp{UnaryOp::Negate,
    //                 ConvertStackEntryToAST(raw_cond)}; cond =
    //                 std::make_unique<AST>(unOp);
    //             }
    //         }
    //         AST::If ifStatement{cond, then};
    //         return std::make_shared<AST>(ifStatement);
    ...
    //         BinaryOp::NotEqual, ConvertStackEntryToAST(val2)}; ASTPtr cond =
    //         std::make_shared<AST>(binaryOp); int64_t pos = m_CurrentPos +
    //         instruction.Op1.addr; std::vector<ASTPtr> then;
    ...
    //         StackEntry val1 = PopStack();
    //         AST::BinaryOperation binaryOp{ConvertStackEntryToAST(val1),
    //         BinaryOp::Equal, ConvertStackEntryToAST(val2)}; ASTPtr cond =
    //         std::make_shared<AST>(binaryOp); int64_t pos = m_CurrentPos +
    //         instruction.Op1.addr; std::vector<ASTPtr> then;
    ...
    //     case OPCODE_NEW: {
    //         auto obj = GetConstantClassFromClass(m_ClassFile,
    //         instruction.Op1.value); StackEntry::New newObj{obj};
    ...
    //     the second value case OPCODE_DUP_X1: {
    //         StackEntry val1 = PopStack();
    //         StackEntry val2 = PopStack();
    ...
    //     }
    //     //Process opcode for Duplicating the top value and inserting it below
    //     the third value case OPCODE_DUP_X2: {
    //         StackEntry val1 = PopStack();
    //         StackEntry val2 = PopStack();
    ...
    //         break;
    //     }
    //     //Process opcode for duplicating the top two values and inserting
    //     them below the third value case OPCODE_DUP2_X1: {
    //         StackEntry val1 = PopStack();
    //         StackEntry val2 = PopStack();
    //         StackEntry val3 = PopStack();
    //         m_Stack.emplace_back(StackEntry(val2));
