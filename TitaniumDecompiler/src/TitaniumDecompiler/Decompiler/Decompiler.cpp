...
void Decompiler::ProcessFunctions(const std::vector<Function>& functions) {
    for (auto& func : functions) {
        ...
        TD_DECOMP_WARN("ssa cfg: {0}", funct.GetFunctionCFG()->m_SsaCfg->toString());
        ...
        // // std::cout << "Number of blocks in CFG: " << funct.GetFunctionCFG()->GetBlocks().size() << std::endl;
        // // domHelper.Print();
        ...
        // std::shared_ptr<ASTNode> root = domHelper.BuildASTFromDomTree(funct.GetFunctionCFG()->GetFirst(), doms, basicBlocks);
...
std::string Decompiler::GenerateJavaCode() {
    ...
    for (auto& func : m_Functions) {
        MethodDescriptor desc = MethodDescriptor::newDescriptor(func.GetMethodDesc());
        std::stringstream ss;
        ...
        javaCode += func.GetMethodAccessFlags() + " fun " + func.GetFunctionName() + "(" + methodDesc + ") {\n";
        ...
        //         std::vector<std::shared_ptr<BasicBlock>> successors = block->GetSuccessor();
        //         if (!successors.empty()) {
...
std::string Decompiler::ConvertFunctionToJava(const Function& func, std::unordered_map<int, StackEntry>& localVars) {
    std::string javaCode = "";
    Function funct = func;
    ...
    if (funct.GetMethodAccessFlags().find("static") != std::string::npos) {
        ...
        std::string className = GetConstantClassFromClass(funct.GetFunctionClassFile(), funct.GetFunctionClassFile().m_ThisClass);
...
std::string Decompiler::GetConstantUTF8FromClass(const ClassFile& classFile, int idx) {
...
std::string Decompiler::GetConstantClassFromClass(const ClassFile& classFile, int idx) {
    int constantIdx = idx - 1;
    auto cpInfo = classFile.m_ConstantPool.m_ConstPoolInfo.at(constantIdx);
...
std::string Decompiler::GetNameAndTypeFromClass(const ClassFile& classFile, int idx) {
    int constantIdx = idx - 1;
    auto cpInfo = classFile.m_ConstantPool.m_ConstPoolInfo.at(constantIdx);
...
std::string Decompiler::GetConstantFromClass(const ClassFile& classFile, int idx) {
    int constantIdx = idx - 1;
    auto cpInfo = classFile.m_ConstantPool.m_ConstPoolInfo.at(constantIdx);
    auto* classInfo = cpInfo.asClassInfo();
    std::string className = GetConstantUTF8FromClass(classFile, classInfo->nameIndex);
...
std::string Decompiler::ConvertConstToName(const ClassFile& classFile, int idx) {
    std::string output = "";
    ...
    if (info.Tag == Tags::Field) {
        ...
        std::string nameInf = GetNameAndTypeFromClass(classFile, field->nameAndTypeIndex);
        ...
        std::string nameInf = GetNameAndTypeFromClass(classFile, method->nameAndTypeIndex);
