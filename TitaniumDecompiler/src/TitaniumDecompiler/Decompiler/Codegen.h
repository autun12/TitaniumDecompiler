...
private:
    ...
    uint32_t FindStartNodeId(
        const std::vector<std::shared_ptr<BasicBlock>>& blocks);
    std::optional<AST> ReadInstruction(const Function& function,
    ...
    ASTPtr ConvertConditionToAST(const Insn& instruction,
                                 std::vector<StackEntry>& stack);
    ASTPtr ConvertCFGToAST(
        std::shared_ptr<BasicBlock> block,
        const std::map<int, std::shared_ptr<BasicBlock>>& cfgBlocks,
        const std::map<uint32_t, uint32_t>& dominators);
    bool IsBackEdge(const Insn& instruction,
