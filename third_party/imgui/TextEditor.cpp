                        ...
                        if (singleStartStr.size() > 0 &&
                            ...
                            withinSingleLineComment = true;
                                   ...
                                   currentIndex + startStr.size() <=
                                       line.size() &&
                                   equals(startStr.begin(), startStr.end(),
                                          from, from + startStr.size(), pred)) {
                            commentStartLine = currentLine;
                            commentStartIndex = currentIndex;
                        }
...
TextEditor::LanguageDefinition::CPlusPlus() {
    ...
    if (!inited) {
        ...
        langDef.mTokenize = [](const char* in_begin, const char* in_end,
            ...
            } else if (TokenizeCStyleString(in_begin, in_end, out_begin,
                                            out_end))
...
const TextEditor::LanguageDefinition& GetJVMLanguageDefinition() {
    static bool inited = false;
    static TextEditor::LanguageDefinition langDef;
    if (!inited) {
        // 1. JVM Opcodes (Keywords)
        ...
        for (auto& k : opcodes) langDef.mKeywords.insert(k);

        // 2. Map syntax elements using regex matching rules
        // Strings (like pool constants)
        langDef.mTokenRegexStrings.push_back(
            std::make_pair<std::string, TextEditor::PaletteIndex>(
                "\\\"(\\\\.|[^\\\"])*\\\"", TextEditor::PaletteIndex::String));

        // Hexadecimal and Decimal Constants (Offset indexes and integer
        // operands)
        langDef.mTokenRegexStrings.push_back(
            std::make_pair<std::string, TextEditor::PaletteIndex>(
        ...
        // JVM Type descriptors (e.g., Ljava/lang/String; or [I)
        // langDef.mTokenRegexStrings.push_back(
        //     std::make_pair<std::string, TextEditor::PaletteIndex>(
        //         "L[a-zA-Z_][a-zA-Z0-9_/]*;",
        //         TextEditor::PaletteIndex::KnownId));

        // Assembly labels/locations (e.g., L0:, label_4:)
        langDef.mTokenRegexStrings.push_back(
            std::make_pair<std::string, TextEditor::PaletteIndex>(
                "[a-zA-Z_][a-zA-Z0-9_]*:",
                TextEditor::PaletteIndex::Identifier));

        // Punctuation (Brackets, arrows, etc.)
        langDef.mTokenRegexStrings.push_back(
            std::make_pair<std::string, TextEditor::PaletteIndex>(
                "[\\[\\]\\{\\}\\!\\%\\^\\&\\*\\(\\)\\-\\+\\=\\~\\|\\<\\>\\?\\/"
                "\\;\\,\\.]",
        ...
        // JVM single line comments (usually starting with '//' or ';')
        langDef.mSingleLineComment = "//";

        langDef.mCaseSensitive = true;
        langDef.mAutoIndentation = false;
        langDef.mName = "JVM Bytecode";
