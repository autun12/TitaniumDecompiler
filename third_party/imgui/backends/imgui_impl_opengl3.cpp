...
//  ImGuiPlatformIO interface. 2026-09-07: Round framebuffer dimensions to the
//  nearest integer instead of truncating them. (#9538, 9515, #8628) 2026-07-15:
//  OpenGL: Backup and restore GL_UNPACK_ROW_LENGTH and GL_UNPACK_ALIGNMENT in
//  UpdateTexture() to avoid corrupting caller GL state. (#8802, #9473)
//  2026-06-17: OpenGL: Expose selected render state in
//  ImGui_ImplOpenGL3_RenderState, Allowing to dynamically select between use of
//  glBindSampler() and glTexParameter(). You can access in 'void*
//  platform_io.Renderer_RenderState' during rendering. 2026-06-03: OpenGL: GLSL
//  version detection assume GLSL 410 when GL context is 4.1. Fixes an issue
//  running on macOS with Wine. (#9427, #6577) 2026-04-23: OpenGL: Added support
//  for standard draw callbacks (in platform_io): DrawCallback_ResetRenderState,
//  DrawCallback_SetSamplerLinear, DrawCallback_SetSamplerNearest. (#9378)
//                      (Breaking): this change prioritize using glBindSampler()
//                      when available, which would override glTexParameter()
//                      settings you may have set on custom textures.
...
static void ImGui_ImplOpenGL3_SetupRenderState(
    ImDrawData* draw_data, ImGui_ImplOpenGL3_RenderState* render_state,
    ...
    if (render_state->UseBindSampler) {
        ...
        glBindSampler(
            0, render_state
                   ->CurrentSampler);  // We use combined texture/sampler state.
                                       // Applications using GL 3.3 and GL
...
// Draw callbacks
static void ImGui_ImplOpenGL3_DrawCallback_ResetRenderState(const ImDrawList*,
                                                            const ImDrawCmd*) {
}  // Intentionally empty. Used as an identifier for rendering loop to call its
   // code. Simpler to implement this way.
static void ImGui_ImplOpenGL3_DrawCallback_SetSamplerLinear(const ImDrawList*,
                                                            const ImDrawCmd*) {
...
static void ImGui_ImplOpenGL3_DrawCallback_SetSamplerNearest(const ImDrawList*,
                                                             const ImDrawCmd*) {
    ImGui_ImplOpenGL3_RenderState* render_state =
        ImGui_ImplOpenGL3_GetRenderState();
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data) {
    ...
    int fb_width =
        (int)(draw_data->DisplaySize.x * draw_data->FramebufferScale.x + 0.5f);
    int fb_height =
        (int)(draw_data->DisplaySize.y * draw_data->FramebufferScale.y + 0.5f);
    ...
    ImGui_ImplOpenGL3_SetupRenderState(draw_data, &render_state, fb_width,
                                       fb_height, vertex_array_object);
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (pcmd->UserCallback ==
                    ...
                    ImGui_ImplOpenGL3_SetupRenderState(draw_data, &render_state,
                                                       fb_width, fb_height,
                                                       vertex_array_object);
                else
                ...
                // Emulate sampler change (even though it is technically part of
                // texture data) As a sort of hack/workaround, we only start
                // writing using glTexParameter() if sampler is ever changed
                // explicitly.
                if (render_state.UseTexParameterFilter) {
                    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                                    render_state.CurrentTexParameterFilter);
    ...
    if (bd->HasBindSampler) {
        ...
        for (int sampler_n = 0; sampler_n < 2; sampler_n++) {
            GL_CALL(glSamplerParameteri(
                bd->TexSamplers[sampler_n], GL_TEXTURE_MIN_FILTER,
                (sampler_n == 0) ? GL_LINEAR : GL_NEAREST));
            GL_CALL(glSamplerParameteri(
                bd->TexSamplers[sampler_n], GL_TEXTURE_MAG_FILTER,
                (sampler_n == 0) ? GL_LINEAR : GL_NEAREST));
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version) {
    ...
    */
    ...
    platform_io.DrawCallback_SetSamplerLinear =
        ImGui_ImplOpenGL3_DrawCallback_SetSamplerLinear;
    platform_io.DrawCallback_SetSamplerNearest =
        ImGui_ImplOpenGL3_DrawCallback_SetSamplerNearest;
