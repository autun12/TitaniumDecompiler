...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_ScrollX =
        1 << 24,  // Enable horizontal scrolling. Require 'outer_size' parameter
                  ...
                  // default sizing policy. Because this create a child window,
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12,  // TableHeadersRow() will not submit label for this column.
                  // Convenient for some small columns. Name will still appear
                  // in context menu.
...
// ImGuiTableColumnFlags_WidthFixed |
// ImGuiTableColumnFlags_NoResize, // Column will not stretch
// and keep resizing based on submitted contents.
#endif
};
...
enum ImGuiDockNodeFlags_ {
    ...
    ImGuiDockNodeFlags_KeepAliveOnly =
        1 << 0,  // Shared       // Don't display the dockspace node but keep it
                 // alive. Windows docked into this dockspace node won't be
                 // undocked.
    ...
    ImGuiDockNodeFlags_PassthruCentralNode =
        1 << 3,  // Shared       // Enable passthru dockspace: 1) DockSpace()
                 ...
                 // everything excepted the Central Node when empty. Meaning the
                 // host window should probably use SetNextWindowBgAlpha(0.0f)
    ...
    ImGuiDockNodeFlags_NoSplit =
        1 << 4,  // Shared/Local // Disable splitting the node into smaller
                 ...
                 // root one (the root one may have splitting disabled to reduce
                 // confusion). Note: when turned off, existing splits will be
                 // preserved.
    ImGuiDockNodeFlags_NoResize =
        1 << 5,  // Shared/Local // Disable resizing node using the
                 // splitter/separators. Useful with programmatically setup
                 // dockspaces.
    ImGuiDockNodeFlags_AutoHideTabBar =
        1 << 6  // Shared/Local // Tab bar will automatically hide when there is
...
struct ImGuiStyle {
    ...
    float LayoutAlign;   // Element alignment inside horizontal and vertical
                        // layouts (0.0f - left/top, 1.0f - right/bottom, 0.5f -
                        // center).
...
struct ImGuiIO {
    ...
    int KeyMap[ImGuiKey_COUNT];  // [LEGACY] Input: map of indices into the
                                 ...
                                 // >512.
    bool KeysDown[512];  // [LEGACY] Input: Keyboard keys that are pressed
...
struct ImGuiTableColumnSortSpecs {
    ...
    ImGuiSortDirection SortDirection
        : 8;  // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending
              // (you can use this or SortSign, whichever is more convenient for
              // your sort function)
...
struct ImFontConfig {
    ...
    int OversampleH;   // 3        // Rasterize at higher quality for sub-pixel
                       ...
                       // minimal so you can reduce this to 2 to save memory.
                       // Read
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_NoAutoMerge =
        1 << 11,  // Platform Window: Avoid merging this window into another
                  ...
                  // viewport flags override (because we need to now ahead if we
                  // are going to create a viewport in the first place!).
