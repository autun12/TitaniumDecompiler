...
public:
    ...
    bool IsStreamGood() const { return stream.good(); }

    uint64_t GetStreamPosition() { return stream.tellg(); }

    void SetStreamPosition(uint64_t pos) { stream.seekg(pos); }
...
protected:
    ...
    void SetUp() override {
        buffer =
            std::stringstream(std::ios::in | std::ios::out | std::ios::binary);
