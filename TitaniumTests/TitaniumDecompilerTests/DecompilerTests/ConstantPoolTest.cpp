...
class MockBigEndianStreamReader
    : public TitaniumDecompiler::BigEndianStreamReader {
public:
    std::vector<uint8_t> buffer;
    size_t pos = 0;

    explicit MockBigEndianStreamReader(const std::vector<uint8_t>& data)
        : buffer(data), pos(0) {}
    ...
    void SetStreamPosition(uint64_t position) override { pos = position; }
...
std::shared_ptr<NameAndTypeInfo> CreateMockNameAndTypeInfo(uint16_t nameIdx,
                                                           uint16_t descIdx) {
...
TEST(ConstantPoolTest, GetConstantUTF8_ReturnsExpectedString) {
    ...
    std::string result = pool.GetConstantUTF8(1);  // 1-based index
...
TEST(ConstantPoolTest, GetConstantClass_ReturnsClassNameFromUtf8) {
    ...
    std::string result = pool.GetConstantClass(2);
...
TEST(ConstantPoolTest, GetNameAndType_ReturnsFormattedString) {
    ...
    auto result = pool.GetNameAndType(3);  // index 3 (1-based)
    EXPECT_EQ(result.first + result.second, "methodName:()V");
}
...
TEST(ConstantPoolTest, GetConstant_ReturnsCorrectEntry) {
    ...
    const ConstPoolInfo* returned = pool.GetConstant(0);
    auto utf8 = returned->GetAs<UTF8Info>();
...
TEST(ConstantPoolTest, GetConstantUTF8_ThrowsForInvalidIndex) {
    ConstantPool pool;
    std::string utf8Const = pool.GetConstantUTF8(1);
    EXPECT_THROW(utf8Const, std::out_of_range);
}
...
TEST(ConstantPoolTest, Deserialize_SkipsLongAndDoubleEntries) {
    ...
    // FIXED: Expect only 3 entries — the Long tag occupies 2 slots, but only 1
    // is pushed
