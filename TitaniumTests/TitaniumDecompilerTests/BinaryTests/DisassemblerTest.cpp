...
#include "TitaniumDecompiler/src/Platform/Linux/FileFormats/JVM/Attributes.h"
...
class DisassemblerTest : public ::testing::Test {
    protected:
        ...
        std::filesystem::path CreateTempFile(const std::string& content, const std::string& extension = ".class") {
            std::filesystem::path tempPath = std::filesystem::temp_directory_path() / ("temp_test_file" + extension);
    ...
    TEST_F(DisassemblerTest, DisassembleValidClassFile) {
        ...
        std::vector<uint8_t> classBytes((std::istreambuf_iterator<char>(inFile)), std::istreambuf_iterator<char>());
    
        // Parse and heap-allocate ClassFile
        auto parser = std::make_unique<TitaniumDecompiler::ClassFileParser>();
        std::shared_ptr<TitaniumDecompiler::ClassFile> classFile = std::make_shared<TitaniumDecompiler::ClassFile>(parser->Parse(classBytes));
    
        // Get class name safely
        std::string className = TitaniumDecompiler::GetConstantClass(classFile->m_ThisClass);
    
        // Disassemble
        TitaniumDecompiler::Disassembler dis;
        std::string output = dis.DisassembleClassFile(*classFile, className);  // Pass by ref to avoid copy
    ...
    TEST_F(DisassemblerTest, DisassembleInvalidClassFile) {
        ...
        std::string corrupted(reinterpret_cast<const char*>(invalidData.data()), invalidData.size());
    ...
    TEST_F(DisassemblerTest, DisassembleValidJarFile) {
        ...
        //std::string validJarContent = "valid jar content"; // Replace with actual valid jar content
        //std::filesystem::path tempPath = CreateTempFile(validJarContent);
        std::filesystem::path testFilePath = "JavaTestFiles/TestJarFile/TestJarFile.jar";
    ...
    TEST_F(DisassemblerTest, CreateFunctions) {
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(nameEntry));// index 0
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(descEntry)); // index 1
    ...
    TEST_F(DisassemblerTest, ParseFunctionInstructions) {
        ...
        std::string output = disassembler.ParseFunctionInstructions(function, classFile, instructions);
        EXPECT_FALSE(output.empty());
    }
    ...
    TEST_F(DisassemblerTest, GetConstantUTF8FromClass) {
        ...
        std::string utf8String = disassembler.GetConstantUTF8FromClass(classFile, 1);
    ...
    TEST_F(DisassemblerTest, GetConstantClassFromClass) {
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(utf8Entry));  // index 0
    
        // Step 2: Add Class entry pointing to UTF8 index 1 (JVM uses 1-based indexing)
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(classEntry)); // index 1
    
        // Step 3: Use index 2 (points to entry at [1]) to get the class name
        std::string className = disassembler.GetConstantClassFromClass(nullptr, classFile, 2);
    ...
    TEST_F(DisassemblerTest, GetNameAndTypeFromClass) {
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(nameUtf8Entry)); // index 0
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(descUtf8Entry)); // index 1
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(ntEntry)); // index 2
    
        // Step 4: Call with idx = 3, which points to index 2 (NameAndType)
        std::string nameAndTypeStr = disassembler.GetNameAndTypeFromClass(nullptr, classFile, 3);
    ...
    TEST_F(DisassemblerTest, InstructionsWithReferences) {
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(classNameEntry)); // index 0
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(fieldNameEntry)); // index 2
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(fieldDescEntry)); // index 3
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(nameTypeEntry)); // index 4
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(fieldRefEntry)); // index 5
    
        // Step 7: Set up instruction to reference the FieldRef at index 6 (JVM-style)
        ...
        std::string output = disassembler.InstructionsWithReferences(function, classFile, insn);
        EXPECT_FALSE(output.empty());
    ...
    TEST_F(DisassemblerTest, LoadConstFromPool) {
        ...
        classFile.m_ConstantPool.m_ConstPoolInfo.push_back(std::move(stringEntry)); // index 1
        ...
        std::string output = disassembler.LoadConstFromPool(function, classFile, insn);
