...
void GraphViewPanel::OnImGuiRender(bool* open) {
    ImGui::Begin("Flow Graph", open);
    ...
    for (const auto& edge : m_Edges) {
        ImVec2 from_pos = (ImVec2(edge->from->m_Pos.x + edge->from->m_Size.x / 2, edge->from->m_Pos.y + edge->from->m_Size.y / 2) * m_GraphZoom) + origin;
        ImVec2 to_pos = (ImVec2(edge->to->m_Pos.x + edge->to->m_Size.x / 2, edge->to->m_Pos.y + edge->to->m_Size.y / 2) * m_GraphZoom) + origin;
        draw_list->AddLine(from_pos, to_pos, IM_COL32(255, 255, 0, 255), 2.0f); // Default edge color
    }
    ...
    for (const auto& node : m_Nodes) {
        ...
        draw_list->AddRectFilled(node_pos, node_pos + node_size, IM_COL32(128, 128, 128, 255));
        draw_list->AddRect(node_pos, node_pos + node_size, IM_COL32(0, 0, 0, 255));
        ImVec2 text_pos = node_pos + ImVec2(5.0f, 5.0f);
        draw_list->AddText(text_pos, IM_COL32(255, 255, 255, 255), node->m_Label.c_str());

        // Interaction (Selection example - using block address as ID)
        ImGui::SetCursorScreenPos(node_pos);
        ImGui::InvisibleButton(std::to_string(reinterpret_cast<uintptr_t>(node->block.get())).c_str(), node_size);
        if (ImGui::IsItemHovered()) {
            draw_list->AddRect(node_pos - ImVec2(2, 2), node_pos + node_size + ImVec2(2, 2), IM_COL32(255, 255, 255, 255));
...
void GraphViewPanel::CreateGraphDataFromCFG(TitaniumDecompiler::CFG& cfg) {
    ...
    std::map<std::shared_ptr<TitaniumDecompiler::BasicBlock>, float> nodeXPositions;
    std::map<int, std::vector<std::shared_ptr<TitaniumDecompiler::BasicBlock>>> blocksByDepth;
    ...
    std::shared_ptr<TitaniumDecompiler::BasicBlock> startBlock = cfg.GetFirst(); // Assuming you have a way to get the starting block
    ...
    if (startBlock) {
        ...
        while (!queue.empty()) {
            std::shared_ptr<TitaniumDecompiler::BasicBlock> currentBlock = queue.front();
            ...
            for (const auto& successorEx : currentBlock->GetSuccessorExceptions()) {
        ...
        // If no starting block, just assign depth 0 to all (will likely overlap)
    ...
    for (const auto& depthPair : blocksByDepth) {
        ...
        for (const auto& blockPtr : blocksAtDepth) {
            ...
            node->m_Label = ss.str();
            node->m_Size = ImVec2(150.0f, std::max(40.0f, static_cast<float>(blockPtr->GetInstructions().size() * 15.0f)));
        ...
        for (size_t i = 0; i < blocksAtDepth.size(); ++i) {
            nodeXPositions[blocksAtDepth[i]] = current_x + nodeSizesAtDepth[i].x / 2.0f;
            current_x += nodeSizesAtDepth[i].x + 50.0f;
        }

        for (const auto& blockPtr : blocksAtDepth) {
            if (m_BlockToNode.count(blockPtr)) {
                m_BlockToNode[blockPtr]->m_Pos = ImVec2(nodeXPositions[blockPtr] * 1.5f, y_offset + depth * depth_spacing); // Adjust multipliers as needed
            }
        }
    ...
    for (const auto& blockPtr : basicBlocks) {
        if (m_BlockToNode.count(blockPtr)) {
            ...
            for (const auto& successorExceptionPtr : blockPtr->GetSuccessorExceptions()) {
